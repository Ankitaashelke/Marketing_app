...
def main():
    ...
    file_url ='https://github.com/Ankitaashelke/Marketing_app/blob/master/Week11_dataset.csv'
    ...
    if st.button("Show Statewise Product Sales"):
        state_product_sales= df[df['ship-state']== selected_state].groupby(['Category'])['Amount'].sum().reset_index()
        state_product_sales =state_product_sales.sort_values(by='Amount', ascending=False)

        plt.figure(figsize=(10, 6))

        sns.lineplot(data=state_product_sales, x='Category', y='Amount',palette='viridis')

        plt.title(f'Product Sales in {selected_state}')
